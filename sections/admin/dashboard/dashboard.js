...
const Dashboard = ({ }) => {
    ...
    useEffect(() => {
        const getData = async () => {
            const data1 = await getAdminDashboardData();
                                ...
                                <View className={`flex-1 px-4`}>
                                    ...
                                    <View className={`h-full w-full p-6 rounded-3xl border border-[#F1F1F1] bg-[#FAFAFC] flex flex-col justify-between`}>
                                        <Text className="font-medium pb-4 text-lg leading-none" style={{
                                            ...
                                            lineHeight: 20,
                                        }} >Students & Teachers</Text>
                                        <View style={styles.dataContainer}>
                                            <Text style={styles.dataLabel}>Total Students:</Text>
                                            <Text style={styles.dataValue}>{data?.totalstudent}</Text>
                                        </View>
                                        <View style={styles.dataContainer}>
                                            <Text style={styles.dataLabel}>Total Teachers:</Text>
                                            <Text style={styles.dataValue}>{data?.totalteacher}</Text>
                                        </View>
                                    </View>
                                </View>
